function tex_actual_hatch(obj, pattern, color, style, step, width)
	switch pattern
		case 'nwse'
			tex_actual_hatch(obj, -45, color, style, step, width);
		case 'swne'
			tex_actual_hatch(obj, 45, color, style, step, width);
		case 'hrzt'
			tex_actual_hatch(obj, 0.001, color, style, step, width);
		case 'vert'
			tex_actual_hatch(obj, 90.001, color, style, step, width);
		case 'cross'
			tex_hatch(obj, 'nwse', color, style, step, width);
			tex_hatch(obj, 'swne', color, style, step, width);
		case 'sqrt'
			tex_hatch(obj, 'hrzt', color, style, step, width);
			tex_hatch4(obj, 'vert', color, style, step, width);
		otherwise
			'''nwse''|''swne''|''hrzt''|''vert''|''cross''|''sqrt'''
	end
end

function tex_hatch(obj,angle,color,style,step,width)
  # tex_hatchfill  Hatches a two-dimensional domain.
  ...
  #   tex_hatchfill(OBJ) Hatchs the domain bounded by the Xdata and Ydata
  #     properties of the OBJ (can be line or patch type).
  #   tex_hatchfill by itself takes as OBJ the last object created in the
  #     current axes.
  #   tex_hatchfill(OBJ,ANGLE,COLOR,STYLE,STEP,WIDTH) Specifies additional
  ...
  #   tex_hatchfill(OBJ,[ANGLE STEP WIDTH],[COLOR STYLE]), where the last
  #     argument is a string: 'w--', '-.y' are both legal notations.
  #   LH = tex_hatchfill(OBJ) also returns the handle of a hatch line.
  ...
  #  May 10, 2001
  #
  #  port to octave 12July2022 dirk mayhew
  ...
  if argcount<6, width = widthdflt; endif
  if argcount<5, step = stepdflt;   endif
  if argcount<4, style = styledflt; endif
  if argcount<3, color = colordflt; endif
  if argcount<2, angle = angledflt; endif

  # Check for step and width in one vector
  if length(angle)>1
    step = angle(2);
    if length(angle)>2, width = angle(3); endif
    angle = angle(1);
  endif
  ...
  if isstr(color)
    ...
    n0 = find(any(A));
    str = color(any(A)==0);
    if ~isempty(n0), color = color(n0); endif
    if ~isempty(str)
      iss = strcmp(str,'-')|strcmp(str,'--')|strcmp(str,'-.');
      if iss|strcmp(str,':'), style = str; endif
    endif
  endif
